...
# 使用攝影機作為輸入
cap = cv2.VideoCapture(1, cv2.CAP_DSHOW) # 0 表示預設攝影機
...
def calculate_intersection(slope1, intercept1, slope2, intercept2):
    ...
    :return: (x, y) 交點座標，或 None
    """
    if intercept1 is None or intercept2 is None:
        return None  # 避免 NoneType 運算錯誤

    if slope1 is None:  # 第一條是垂直線
        x = intercept1
        y = slope2 * x + intercept2 if slope2 is not None else intercept2
    elif slope2 is None:  # 第二條是垂直線
        x = intercept2
        y = slope1 * x + intercept1 if slope1 is not None else intercept1
    else:
...
def calculate_intercept(slope, point):
    ...
    :return: 截距 b，如果斜率為 None（垂直線），返回 x 座標
    """
    if slope is None:  # 垂直線無法計算截距，但可以回傳 x 座標
        return point[0]
    x, y = point
    return y - slope * x
